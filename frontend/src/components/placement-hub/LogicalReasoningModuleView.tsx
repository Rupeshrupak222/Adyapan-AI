...
export function LogicalReasoningModuleView({ setView, theme = "dark" }: LogicalReasoningModuleViewProps) {
  ...
  return (
    <div
      className="min-h-screen p-4 sm:p-6 md:p-8 space-y-8 font-sans antialiased"
    ...
    >
      ...
      <div className="flex items-center gap-2 text-xs font-semibold text-slate-400">
        <span
          ...
          onClick={() => setView && setView("placement-hub")}
        >
          Placement Hub
        </span>
        <ChevronRight size={14} className="text-slate-600" />
        <span className="text-amber-400 font-bold">Logical Reasoning</span>
      </div>
      ...
      <motion.div
        ...
        className="relative overflow-hidden rounded-[20px] p-6 sm:p-8 border flex flex-col md:flex-row items-center justify-between gap-8"
        style={{
          background: "linear-gradient(135deg, rgba(245, 158, 11, 0.12) 0%, rgba(15, 23, 42, 0.95) 60%, rgba(15, 23, 42, 1) 100%)",
      ...
      >
        ...
        <div className="space-y-4 max-w-xl z-10">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold bg-amber-500/10 border border-amber-500/30 text-amber-400 uppercase tracking-widest">
            <Flame size={14} className="animate-pulse text-amber-400" /> AI Placement Module
          </div>
          <h1 className="text-2xl sm:text-4xl font-extrabold tracking-tight font-sans text-white">
          ...
          <div className="flex flex-wrap items-center gap-3 pt-2">
            ...
            <motion.button
              ...
              className="px-5 py-2.5 rounded-xl font-bold text-xs bg-white/10 hover:bg-white/15 border border-white/10 text-white flex items-center gap-2 backdrop-blur-md transition-all"
            >
              <Sparkles size={14} className="text-amber-400" /> Generate AI Quiz
            </motion.button>
          </div>
        </div>

        {/* Right Animated Hero Illustration */}
        <div className="relative z-10 shrink-0 w-full md:w-72 h-44 sm:h-52 rounded-2xl flex items-center justify-center border border-amber-500/20 bg-slate-900/60 backdrop-blur-xl overflow-hidden group">
          <div className="absolute inset-0 bg-gradient-to-tr from-amber-500/10 to-transparent pointer-events-none" />
          {/* Animated Float Nodes */}
          <motion.div
            animate={{ y: [0, -8, 0], rotate: [0, 5, 0] }}
            transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
            className="absolute top-4 left-4 p-2.5 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-400"
          >
            <Brain size={22} />
          </motion.div>
          <motion.div
            ...
            className="absolute bottom-4 right-4 p-2.5 rounded-xl bg-amber-500/15 border border-amber-500/30 text-amber-400"
          >
            <Puzzle size={22} />
          </motion.div>
          <motion.div
          ...
          >
            ...
            <p className="text-xs font-bold text-white">1,400+ Placement Questions</p>
            <p className="text-[10px] text-slate-400 font-semibold mt-0.5">TCS • Infosys • Amazon • Google</p>
          </motion.div>
        </div>
      </motion.div>
      ...
      <div
        className="rounded-[20px] p-5 border space-y-3"
      ...
      >
        ...
        <div className="relative flex items-center gap-2">
          <input
            ...
            className="w-full rounded-xl px-4 py-3 text-xs bg-slate-900/90 border border-slate-700/80 text-white placeholder-slate-500 focus:outline-none focus:border-amber-500 transition-all"
          />
          <motion.button
            ...
            className="px-5 py-3 rounded-xl font-bold text-xs bg-amber-500 hover:bg-amber-400 text-slate-950 flex items-center gap-2 shrink-0 transition-all disabled:opacity-50"
          >
            {aiGenerating ? (
              <>
        ...
        <div className="flex items-center gap-2 overflow-x-auto pb-1 scrollbar-none text-[11px]">
          ...
          {PROMPT_SUGGESTION_CHIPS.map((chip) => (
            <button
              ...
              className="px-3 py-1 rounded-lg bg-white/5 hover:bg-amber-500/10 border border-white/10 hover:border-amber-500/30 text-slate-300 hover:text-amber-400 shrink-0 transition-all font-medium"
            >
              {chip}
            </button>
          ))}
      ...
      {/* ── SEARCH & FILTERS BAR ────────────────────────────────────────────── */}
      <div
        className="rounded-[20px] p-4 border flex flex-col md:flex-row items-center justify-between gap-4 relative z-20"
        style={{ background: c.cardBg, borderColor: c.cardBorder }}
      >
        {/* Search Input with Autocomplete */}
        <div className="relative w-full md:w-72">
          <div className="flex items-center gap-2 px-3.5 py-2.5 rounded-xl bg-slate-900/90 border border-slate-700/80 text-white text-xs">
            ...
            />
            {searchQuery && (
              <X size={12} className="text-slate-400 cursor-pointer hover:text-white" onClick={() => setSearchQuery("")} />
            )}
          </div>
              ...
              >
                {autocompleteFiltered.map((item) => (
                  <div
                    ...
                    className="px-3.5 py-2 text-xs text-slate-300 hover:bg-amber-500/10 hover:text-amber-400 cursor-pointer flex items-center justify-between"
                  >
                    <span>{item}</span>
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-slate-400 font-semibold hidden sm:inline">Company:</span>
            <select
              value={selectedCompany}
              onChange={(e) => setSelectedCompany(e.target.value)}
              className="px-3 py-2 rounded-xl text-xs bg-slate-900 border border-slate-700 text-white focus:outline-none focus:border-amber-500"
            >
          ...
          {/* Topic Filter */}
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-slate-400 font-semibold hidden sm:inline">Topic:</span>
            <select
              value={selectedTopic}
          ...
          {/* Difficulty Filter */}
          <div className="flex items-center gap-1.5 text-xs">
            <span className="text-slate-400 font-semibold hidden sm:inline">Difficulty:</span>
            <select
      ...
      <div className="space-y-3">
        <div className="flex items-center justify-between">
          <h2 className="text-sm font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
            <Target size={16} className="text-amber-400" /> Featured Companies
          </h2>
          <span className="text-xs text-slate-400 font-semibold">Scroll left/right →</span>
        </div>
        ...
        <div className="flex items-center gap-4 overflow-x-auto pb-3 scrollbar-none">
          {companies.map((comp, idx) => (
            <motion.div
              ...
              className="w-56 shrink-0 rounded-[20px] p-4 border cursor-pointer transition-all group"
              style={{ background: c.cardBg, borderColor: c.cardBorder }}
            >
              <div className="flex items-center justify-between mb-3">
                <div className="w-10 h-10 rounded-xl bg-amber-500/10 border border-amber-500/20 flex items-center justify-center font-black text-amber-400 text-xs tracking-wider group-hover:scale-110 transition-transform">
                  {comp.logo}
                </div>
                <span
              ...
              <p className="text-[11px] text-slate-400 mt-1 line-clamp-1">{comp.description}</p>
              <div className="mt-3 flex items-center justify-between pt-3 border-t border-white/5 text-[11px]">
                <span className="font-semibold text-slate-400">{comp.questionCount} Questions</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <div className="lg:col-span-2 space-y-8">
          ...
          <div ref={topicsGridRef} className="space-y-4">
            <h2 className="text-sm font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
              <Brain size={16} className="text-amber-400" /> Reasoning Topics (14 Core Placement Categories)
            </h2>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {topics.map((t, idx) => (
                <motion.div
                  ...
                  className="rounded-[20px] p-5 border cursor-pointer transition-all flex flex-col justify-between group hover:border-amber-500/40"
                  style={{ background: c.cardBg, borderColor: c.cardBorder }}
                >
                  <div className="space-y-3">
                    <div className="flex items-center justify-between">
                      ...
                      <div className="relative w-9 h-9 flex items-center justify-center">
                        ...
                        <span className="absolute text-[9px] font-black text-amber-400">{t.progress}%</span>
                      </div>
                    </div>
                    ...
                    <div>
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-[10px] px-2 py-0.5 rounded-full bg-white/5 border border-white/10 text-slate-400 font-semibold">
                          {t.difficulty}
                        </span>
                      </div>
                  ...
                  <div className="mt-4 pt-3 border-t border-white/5 flex items-center justify-between text-xs font-bold">
                    <span className="text-slate-400 font-medium">{t.questionCount} Questions</span>
          ...
          <div ref={practiceRef} className="space-y-4 pt-4">
            <div className="flex items-center justify-between">
              <h2 className="text-sm font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
                <Zap size={16} className="text-amber-400" /> Practice Question Arena
              </h2>
              {questions.length > 0 && (
            ...
            {questionsLoading ? (
              <div className="rounded-[20px] p-12 border text-center space-y-3" style={{ background: c.cardBg, borderColor: c.cardBorder }}>
                <RefreshCw size={24} className="animate-spin text-amber-400 mx-auto" />
                <p className="text-xs font-bold text-slate-400">Loading Reasoning Questions...</p>
              </div>
            ) : currentQuestion ? (
              <motion.div
                ...
                className="rounded-[20px] p-6 border space-y-6 relative"
                style={{ background: c.cardBg, borderColor: c.cardBorder }}
              >
                {/* Question Header & Badges */}
                <div className="flex flex-wrap items-center justify-between gap-3 pb-4 border-b border-white/10">
                  <div className="flex items-center gap-2">
                    <span className="px-2.5 py-1 rounded-lg text-xs font-extrabold bg-amber-500/10 border border-amber-500/20 text-amber-400">
                      {currentQuestion.topic}
                    </span>
                    {currentQuestion.company && (
                      <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-white/5 border border-white/10 text-slate-300">
                        {currentQuestion.company}
                      </span>
                    )}
                    <span className="px-2.5 py-1 rounded-lg text-xs font-bold bg-white/5 border border-white/10 text-slate-400">
                  ...
                  <div className="flex items-center gap-3">
                    <div className="flex items-center gap-1.5 text-xs font-semibold text-slate-400">
                      <Clock size={14} className="text-amber-400" /> {currentQuestion.estimatedTime}
                    </div>
                    <motion.button
                      whileHover={{ scale: 1.1 }}
                      whileTap={{ scale: 0.9 }}
                      onClick={() => handleToggleBookmark(currentQuestion.id)}
                      className="p-2 rounded-xl bg-white/5 border border-white/10 hover:border-amber-500/40 text-slate-300 hover:text-amber-400 transition-colors"
                    >
                ...
                <div className="grid grid-cols-1 gap-3">
                  {currentQuestion.options.map((opt, oIdx) => {
                    ...
                    let optBg = "bg-slate-900/60 border-slate-800 text-slate-300";
                    if (isSelected) {
                      optBg = "bg-amber-500/10 border-amber-500/50 text-amber-400 font-bold";
                    }
                    if (submittedAnswer) {
                      if (isCorrectAnswer) {
                        optBg = "bg-emerald-500/15 border-emerald-500/50 text-emerald-400 font-bold";
                      } else if (isSelected) {
                        optBg = "bg-red-500/15 border-red-500/50 text-red-400 font-bold";
                      }
                    }
                    ...
                    return (
                      <motion.div
                        ...
                        className={`p-4 rounded-xl border text-xs cursor-pointer flex items-center justify-between transition-all ${optBg}`}
                      >
                        <div className="flex items-center gap-3">
                          <span className="w-6 h-6 rounded-lg bg-white/5 border border-white/10 flex items-center justify-center font-bold text-[11px]">
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 pt-2">
                  <div className="flex items-center gap-2">
                    <button
                      ...
                      className="px-3.5 py-2 rounded-xl text-xs font-bold bg-white/5 hover:bg-white/10 border border-white/10 text-slate-300 flex items-center gap-1.5 transition-all"
                    >
                      <Lightbulb size={14} className="text-amber-400" /> {showHint ? "Hide Hint" : "Hint"}
                    </button>
                    {submittedAnswer && (
                      <button
                        onClick={() => setShowExplanation(!showExplanation)}
                        className="px-3.5 py-2 rounded-xl text-xs font-bold bg-amber-500/10 hover:bg-amber-500/20 border border-amber-500/30 text-amber-400 flex items-center gap-1.5 transition-all"
                      >
                ...
                <AnimatePresence>
                  {showHint && (
                    <motion.div
                      ...
                      className="p-4 rounded-xl bg-amber-500/10 border border-amber-500/30 text-xs text-amber-200 leading-relaxed"
                    >
                      <span className="font-extrabold block text-amber-400 mb-1">💡 Hint:</span>
                ...
                <AnimatePresence>
                  {(submittedAnswer || showExplanation) && (
                    <motion.div
                      ...
                      animate={{ opacity: 1, y: 0 }}
                      className="p-5 rounded-xl bg-slate-900/90 border border-slate-700/80 space-y-3 text-xs leading-relaxed"
                    >
                      <div>
                        <span className="font-extrabold text-amber-400 text-xs uppercase tracking-wider block mb-1">
                          🧠 Step-by-Step AI Explanation
                        </span>
                        <p className="text-slate-300 whitespace-pre-line">{currentQuestion.explanation}</p>
                      </div>

                      {currentQuestion.shortcutTrick && (
                        <div className="p-3 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-300 font-medium">
                          <span className="font-bold text-amber-400">⚡ Shortcut Trick: </span>
                          {currentQuestion.shortcutTrick}
                        </div>
                      )}
                    </motion.div>
          ...
          <div
            className="rounded-[20px] p-6 border relative overflow-hidden flex flex-col sm:flex-row items-center justify-between gap-6"
            style={{
          ...
          >
            <div className="space-y-2 max-w-md">
              <div className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-[10px] font-black bg-emerald-500/20 text-emerald-400 uppercase tracking-widest">
              ...
              <div className="flex items-center gap-4 text-xs font-bold text-slate-400 pt-1">
                <span className="text-emerald-400">+50 XP Reward</span>
                <span>•</span>
                <span className="text-amber-400">Difficulty: Hard</span>
                <span>•</span>
        ...
        <div className="space-y-6">
          ...
          <div
            className="rounded-[20px] p-6 border space-y-6"
            style={{ background: c.cardBg, borderColor: c.cardBorder }}
          >
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <h3 className="text-xs font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
                <TrendingUp size={16} className="text-amber-400" /> Performance Summary
              </h3>
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                <span className="text-[10px] font-semibold text-slate-400 uppercase">Questions Solved</span>
                <div className="text-xl font-extrabold text-white">{progress.questionsSolved}</div>
                <span className="text-[9px] text-emerald-400 font-bold">+4 this week</span>
              </div>

              <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                <span className="text-[10px] font-semibold text-slate-400 uppercase">Accuracy Rate</span>
                <div className="text-xl font-extrabold text-amber-400">{progress.accuracy}%</div>
                <span className="text-[9px] text-slate-400 font-semibold">Top 15% percentile</span>
              </div>

              <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                <span className="text-[10px] font-semibold text-slate-400 uppercase">Avg Time / Q</span>
                <div className="text-xl font-extrabold text-white">{progress.avgTimeSeconds}s</div>
                <span className="text-[9px] text-emerald-400 font-bold">12s faster than avg</span>
              </div>

              <div className="p-3.5 rounded-xl bg-slate-900/80 border border-slate-800 space-y-1">
                <span className="text-[10px] font-semibold text-slate-400 uppercase">Daily Streak</span>
                ...
                <span className="text-[9px] text-amber-400 font-semibold">Keep it up!</span>
              </div>
            </div>

            {/* Recharts Weekly Progress Bar Chart */}
            <div className="space-y-2">
              <span className="text-xs font-bold text-slate-300 block">Weekly Progress</span>
            ...
            <div className="space-y-3 pt-2 border-t border-white/10">
              <div>
                <span className="text-xs font-extrabold text-red-400 flex items-center gap-1 mb-2">
                ...
                <div className="space-y-2">
                  {progress.weakTopics.map((wt) => (
                    <div key={wt.topic} className="flex items-center justify-between text-xs">
                      <span className="text-slate-300 font-medium">{wt.topic}</span>
                      <span className="text-red-400 font-bold">{wt.accuracy}% Accuracy</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="pt-2">
                <span className="text-xs font-extrabold text-emerald-400 flex items-center gap-1 mb-2">
                ...
                <div className="space-y-2">
                  {progress.strongTopics.map((st) => (
                    <div key={st.topic} className="flex items-center justify-between text-xs">
                      <span className="text-slate-300 font-medium">{st.topic}</span>
                      <span className="text-emerald-400 font-bold">{st.accuracy}% Accuracy</span>
                    </div>
                  ))}
                </div>
              </div>
          ...
          <div
            className="rounded-[20px] p-6 border space-y-4"
            style={{ background: c.cardBg, borderColor: c.cardBorder }}
          >
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <h3 className="text-xs font-extrabold text-white uppercase tracking-wider flex items-center gap-2">
                <Bookmark size={16} className="text-amber-400" /> Bookmarks & Activity
              </h3>
              <span className="text-xs font-bold text-amber-400">{userBookmarks.size} Saved</span>
            </div>

            <div className="space-y-3 text-xs">
              {progress.recentActivity.map((act) => (
                <div key={act.id} className="p-3 rounded-xl bg-slate-900/60 border border-slate-800 space-y-1">
                  <div className="flex items-center justify-between text-white font-semibold">
                    <span className="truncate max-w-[180px]">{act.title}</span>
                    <span className="text-amber-400 font-bold">{act.score}</span>
                  </div>
                  <span className="text-[10px] text-slate-500 block">{act.timeAgo}</span>
                </div>
              ))}
            </div>
          </div>
