...
async function main() {
  const jsonPath = path.resolve(__dirname, "../../docs/intermediate_dsa_questions_526_549.json");
  console.log(`Reading 526-549 questions JSON from: ${jsonPath}`);
  ...
  console.log(`Loaded ${questions.length} intermediate questions from JSON.`);
  ...
  async function processQuestion(q: ExtractedQuestion) {
    ...
    processedCount++;
    console.log(`Progress: ${processedCount}/${questions.length} (${q.externalId} ${q.title})`);
  }
