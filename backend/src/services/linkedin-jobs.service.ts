...
export async function deleteSavedJob(userId: string, jobId: string): Promise<void> {
  ...
  await userPrisma.linkedInScrapedJob.deleteMany({
    where: { userId, OR: [{ id: jobId }, { link: jobId }] },
  });
}
